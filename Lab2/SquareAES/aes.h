...
void subByteVector(vector<byte> &state, unsigned int mode);
void aes_encrypt_nround(BYTE* cipher, BYTE* message, BYTE* key,int nround);
void aes_decrypt_nround(BYTE* message, BYTE* cipher, BYTE* key, int nround);
void aesEncryptVector(vector<byte> &cipher, const vector<byte> &message, const vector<byte> &key, int nround);
void aesDecryptVector(vector<byte> &message, const vector<byte> &cipher, const vector<byte> &key, int nround);
void invertKeyExpansion(vector<byte> key, int nround);
...
void aesEncryptVector(vector<byte> &cipher, const vector<byte> &message, const vector<byte> &key, int nround)
{
    byte cipherArr[16], messageArr[16], keyArr[16];

    memcpy(messageArr, &message[0], 16);
    memcpy(keyArr, &key[0], 16);

    aes_encrypt_nround(cipherArr, messageArr, keyArr, nround);
    memcpy(&cipher[0], cipherArr, 16);
}
...
void aesDecryptVector(vector<byte> &message, const vector<byte> &cipher, const vector<byte> &key, int nround)
{
    ...
    memcpy(cipherArr, &cipher[0], 16);
    memcpy(keyArr, &key[0], 16);

    aes_decrypt_nround(messageArr, cipherArr, keyArr, nround);
    memcpy(&message[0], messageArr, 16);
}
...
{
    byte stateArr[16];
    memcpy(stateArr, &state[0], 16);
    sub_bytes(stateArr, mode);
    memcpy(&state[0], stateArr, 16);
}
...
void invertKeyExpansion(vector<byte> &allKeys, const vector<byte> &lastKey, int nround)
{
    ...
    for (int round=nround-1; round>=0; round--) {
        ...
        memcpy(nextFirstCol, expandedKey + (round+1)*16, 4);
        nextFirstCol[0] ^= rcon[round+1];  // step 4

        for (int row=0; row<=3; row++) {
            int pos = round*16 + cell(row,0);
            expandedKey[pos] = lastCol[row] ^ nextFirstCol[row];
        }
    }
...
void aes_decrypt_nround(BYTE* message, BYTE* cipher, BYTE* key, int nround)
{
  ...
  p_round_key = &(expanded_key[nround*16]); // nround = 10 -> 160
  add_round_key(state, p_round_key);
