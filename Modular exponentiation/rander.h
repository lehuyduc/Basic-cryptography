...
// Rander use dynamic memory (gmp_randstate_t),
// so to ensure correctness, it must have canonical form: constructor, copy constructor, destructor, assignment operator =
class Rander
{
private:
...
public:
    ...
    // get number with exactly K bit
    bignum getKbitNum(int kbits)
    {
        string res = "1";
        for (int i=2; i<=kbits; i++)
            res += char(randomm(2) + '0');
