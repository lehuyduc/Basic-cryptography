...
// this class has same performance as gmp while making programming much easier
// in benchmark_bignum.cpp, the difference in speed is < 1%
class bignum
{
private:
    mpz_t value;
...
public:
    ...
    bignum(const string &str, int base=0) {
        mpz_init_set_str(value, str.c_str(), base);
    }

    static void setmpz(mpz_t &dest, const bignum& src) {
        mpz_set(dest, src.value);
    }
    ...
    bignum operator % (int val) const {
        bignum res;
        mpz_tdiv_r_ui(res.value, value, abs(val)); // in C++, x % m = x % abs(m)
        return res;
    }
    ...
    // Sometimes we want to read-only (no change allowed) bignum::value, but it's private.
    // Since mpz_t is actually an array, we cannot return by value
    // because C++ doesn't allow returning an array.
    // Therefore, we return its address. the const keywords ensure that the returned pointer is read-only
