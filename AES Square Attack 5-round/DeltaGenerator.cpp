...
void makeDeltaSet(vector<vector<byte>> &plaintexts, int pos) // active pos
{
    ...
    random_shuffle(rand256.begin(), rand256.end()); // rand256 contains 256 distinct values from 0->255, in random order
    ...
    for (i=1; i<256; i++) {
        for (j=0; j<16; j++)
            plaintexts[i][j] = plaintexts[i-1][j];  // 15 bytes are the same across all 256 plaintexts
        plaintexts[i][pos] = rand256[i];            // 1 byte is active and contain 256 different values
    }
}
...
int main()
{
    ...
    for (int t=1; t<=ntest; t++) {
        ...
        for (int i=0; i<256; i++)
            for (int j=0; j<16; j++) printf("%d ",int(plaintexts[i][j]));
        printf("\n");
    }
