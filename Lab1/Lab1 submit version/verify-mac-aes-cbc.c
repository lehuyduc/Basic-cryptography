...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        printf("Usage: %s plaintext_file mac_value\n", argv[0]);
        exit(-1);
    }

    sprintf(name_plaintext, "%s", argv[1]);
    sprintf(mac_value, "%s", argv[2]);
    ...
    bool validFile = true;
    for (int i=0; i<strlen(mac_value); i++)
        if (res[i]!=mac_value[i]) {
            validFile = false;
            break;
        }

    if (validFile) printf("File is valid\n");
    else printf("File is not valid\n");
    return 0;
}
